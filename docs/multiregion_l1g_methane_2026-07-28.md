...
### 8.1 追補：usableシーンの単帯域候補

2026-08-02に、雲・QA基準を通った7製品（同一撮像重複を除くと3観測ストリップ）だけで、
1600 nm側または2200–2390 nm側のどちらか一方が局所z 3以上となる領域も再集計した。
同一ストリップの重複を除くと、正側OR領域は1,847、同じ処理の逆符号対照は2,199だった。
画像確認用に絞っても正側243、逆符号側287である。さらに単帯域のみの領域へ面積、core、
peak、他帯域の符号、雲・QA境界距離、形状の固定条件を加えると、正側は1600 nm only 2件、
逆符号側は2200 nm only 5件であり、確認済みプルームと呼べる単帯域
候補は得られなかった。

固定R2施設の21×21 pixel窓も、1600 nm側は閾値以上が1画素、2200 nm側は0画素で、単帯域へ
緩和しても3画素以上の連結候補にはならなかった。方法、候補座標、画像、完全な対照集計は
[usableシーン単帯域候補報告](single_band_usable_candidates_2026-08-02.md)に示す。
